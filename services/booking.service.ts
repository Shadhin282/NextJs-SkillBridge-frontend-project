...
export const BookingService = {
  getBooking: async function () {
    ...
    try {
      ...
      return {
        data: null,
        error: { message: "internal error, booking not fetch" },
      };
    }
  },
  ...
  getBookingById: async function (id: string) {
    ...
    try {
      const res = await fetch(
        `https://nextjs-skill-bridge-backend-project.onrender.com/api/bookings/${id}`,
        {
          headers: {
            Cookie: cookieStore.toString(),
          },
  ...
  PostBooking: async function (payload: Booking) {
    try {
      ...
      return {
        data: null,
        error: { message: "Server error booking not able to do" },
      };
    }
  },
};
